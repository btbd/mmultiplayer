...
byte Hook::GetInstructionLength(byte table[], byte *instruction) {
	byte i = table[*instruction++];
	return i < 0x10 ? i : GetInstructionLength(INSTRUCTION_TABLES[i - 0x10], instruction);
}

bool Hook::SetJMP(void *dest, void *src, int nops) {
...
bool Hook::TrampolineHook(void *dest, void *src, void **original) {
	if (!src) {
		return false;
	}
